/*
  # Enhanced African News Aggregator Schema
...
CREATE INDEX IF NOT EXISTS idx_posts_country_focus ON posts USING GIN(country_focus) WHERE country_focus IS NOT NULL;
...
-- Create functional unique index for trending topics daily uniqueness
...
-- Triggers for updated_at (only for new tables)
...
-- Trigger to create user preferences automatically
