...
export default function HomePage() {
  ...
  const loadTrendingContent = async () => {
    try {
      ...
      // Try to load trending topics
      try {
        promises.push(NewsService.getTrendingTopics());
      } catch (topicsError) {
        console.warn("getTrendingTopics not available:", topicsError);
        promises.push(Promise.resolve([]));
      }

      // Try to load trending articles
      try {
        promises.push(NewsService.getArticles({ trending: true, limit: 5 }));
      } catch (articlesError) {
        console.warn("getArticles not available:", articlesError);
        promises.push(Promise.resolve({ articles: [] }));
      }

      const [topics, articles] = await Promise.all(promises);
      ...
      setTrendingTopics([
        { id: 1, name: "Breaking News", count: 12, trend: "up" },
        { id: 2, name: "Technology", count: 8, trend: "stable" },
        { id: 3, name: "Politics", count: 6, trend: "down" },
      ]);
      setTrendingArticles([]);
    } finally {
      setLoading(false);
              ...
              {!user && (
                ...
                >
                  ...
                  <p className="text-orange-100 text-sm mb-4">
                    Get personalized news, bookmark articles, and join the
                    conversation
                  </p>
